...
def test_TypeName_resolves_to_capitalized_symbol_name():
    assertResolvesTo(UnresolvedTypeName, name='cat', givenName='pet', resolvesTo='Cat')


def test_VariableName_resolves_to_symbol_given_name():
    assertResolvesTo(UnresolvedVariableName, name='cat', givenName='pet', resolvesTo='pet')


def test_VariableName_resolves_to_symbol_name_if_no_given_name():
    assertResolvesTo(UnresolvedVariableName, name='cat', givenName=None, resolvesTo='cat')


def test_TypeName_resolves_to_Token_for_terminal_symbols():
    assertResolvesTo(UnresolvedTypeName, name='cat', givenName='pet', isTerminal=True, resolvesTo='Token')


def test_ClassName_resolves_to_symbol_given_name():
    assertResolvesTo(UnresolvedClassName, name='cat', givenName='Pet', resolvesTo='Pet')


def test_ClassName_resolves_to_capitalized_symbol_name_if_no_given_name():
    assertResolvesTo(UnresolvedClassName, name='cat', givenName=None, resolvesTo='Cat')


def test_BaseClassName_resolves_to_capitalized_symbol_name():
    assertResolvesTo(UnresolvedBaseClassName, name='cat', givenName='Pet', resolvesTo='Cat')
