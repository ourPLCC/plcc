...
def test_parse_skip_rule(parser):
    ...
    rules = list(rules)
    assert rules[0] == LexRule(isToken=False, name='WS', pattern=r'\s+', quote="'", end="", line=Line(string=r"skip WS '\s+'", path='', number=1))


def test_parse_token_rule(parser):
    rules = parser.parse(lines(r"token WS '\s+'"))
    rules = list(rules)
    assert rules[0] == LexRule(isToken=True, name='WS', pattern=r'\s+', quote="'", end="", line=Line(string=r"token WS '\s+'", path='', number=1))


def test_parse_implicit_token_rule(parser):
    rules = parser.parse(lines(r"WS '\s+'"))
    rules = list(rules)
    assert rules[0] == LexRule(isToken=True, name='WS', pattern=r'\s+', quote="'", end="", line=Line(string=r"WS '\s+'", path='', number=1))


def test_allow_line_comment_in_lex_rule(parser):
    rules = parser.parse(lines(r"WS '\s+' # hi"))
    rules = list(rules)
    assert rules[0] == LexRule(isToken=True, name='WS', pattern=r'\s+', quote="'", end=" # hi", line=Line(string=r"WS '\s+' # hi", path='', number=1))


def test_allow_pound_in_lex_rule_pattern(parser):
    rules = parser.parse(lines(r"COMMENT ' #\s+' # matching a comment"))
    rules = list(rules)
    assert rules[0] == LexRule(isToken=True, name='COMMENT', pattern=r' #\s+', quote="'", end=" # matching a comment", line=Line(string=r"COMMENT ' #\s+' # matching a comment", path='', number=1))


def test_skip_blank_lines_in_lex(parser):
    rules = parser.parse(lines("\n\nCOMMENT ' #\\s+' # matching a comment"))
    rules = list(rules)
    assert rules[0] == LexRule(isToken=True, name='COMMENT', pattern=r' #\s+', quote="'", end=" # matching a comment", line=Line(string=r"COMMENT ' #\s+' # matching a comment", path='', number=3))
...
'''))
    rules = list(rules)
    assert rules[0] == LexRule(isToken=False, name='WS', pattern=r'\s+', quote='"', end="", line=Line(string=r'skip WS "\s+"', path='', number=2))
    assert rules[1] == LexRule(isToken=True, name='HI', pattern=r'hi', quote="'", end="   # for a greeting", line=Line(string=r"token HI 'hi'   # for a greeting", path='', number=4))
